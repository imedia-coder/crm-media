...
export class PipelineStagesService {
  ...
  findAll() {
    return this.tenantPrisma.client.pipelineStage.findMany({
      orderBy: { order: 'asc' },
    });
  }

  async findOneOrThrow(id: string) {
    const stage = await this.tenantPrisma.client.pipelineStage.findUnique({
      where: { id },
    });
  ...
  async update(id: string, dto: UpdatePipelineStageDto) {
    ...
    return this.tenantPrisma.client.pipelineStage.update({
      where: { id },
      data: dto,
    });
  }
  ...
  async remove(id: string) {
    await this.findOneOrThrow(id);
    const dealsUsingStage = await this.tenantPrisma.client.deal.count({
      where: { stageId: id },
    });
    if (dealsUsingStage > 0) {
      throw new ConflictException(
        'Cannot delete a pipeline stage that still has deals in it',
      );
    }
